...
frappe.ui.form.on("Call Logs", {
	refresh: function(frm) {
		if(!cur_frm.doc.__islocal && cur_frm.doc.phone_number) {
			html = "<a href='vdsip:" + cur_frm.doc.phone_number + "' onclick='cur_frm.cscript.make_new_call_log()' ><i class='icon-phone' aria-hidden='true'></i> <b>Call</b></a>"
		...
		if(cur_frm.doc.client && !cur_frm.doc.__islocal) {
			...
			cur_frm.add_custom_button(__("Show Log"), function() {
				cur_frm.cscript.show_logs();
			});
		}
	}
});

cur_frm.cscript.make_new_call_log = function() {
...
frappe.ui.form.on("Call Logs", {
	validate: function(frm) {
		if(cur_frm.doc.__islocal && cur_frm.doc.start_time) {
			...
			frappe.call({
				method: "switsol_ag.switsol_ag.make_user.update_reference_person_after_making_call_logs",
				args: {
...
cur_frm.cscript.show_logs = function() {
