...
timelog = Class.extend({
	...
	make_timesheet: function() {
		...
		console.log('time')
		frappe.call({
			method: "switsol_ag.time_log_sheet.page.time_log_sheet.time_log_sheet.make_timesheet",
			args: {
	...
	calulate_hours: function() {
		...
		frappe.call({
			method: "switsol_ag.time_log_sheet.page.time_log_sheet.time_log_sheet.calculate_total_hours",
			args: {
	...
	get_loged_sheets: function() {
		...
		$(".logsheet").find("button[data-fieldname='logsheet']").on("click", function() {
			...
			if(logged_date) {
				frappe.call({
					method: "switsol_ag.time_log_sheet.page.time_log_sheet.time_log_sheet.get_loged_timesheets",
					args: {
						"date": logged_date
					},
					callback: function(r) {
						if(r.message) {
							this.dialog = new frappe.ui.Dialog({
								...
								fields: [
									{
										"fieldtype":"HTML",
										"label":__("Loged Timesheets"),
										"reqd":1,
										"fieldname":"loged_sheets"
									}
								]
							});
