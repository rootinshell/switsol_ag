...
def validate(self, method=None):
    ...
    round_to(self)
    show_items = validate_items(self)
    show_additional = validate_additional(self)
    if show_items:
        self.item_details = 1
    if show_additional and not show_items:
        self.item_details = 1
...
def validate_additional(self):
    for item in self.additional:
        if item.description.lower() == "leer":
            ...
            item.show_in_details = 1
    show_in_details = [i for i in self.additional if i.show_in_details]
    if show_in_details:
        return True
        # frappe.db.set(self, "item_details", 1)
    else:
