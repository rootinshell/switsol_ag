...
cur_frm.cscript.clerk_name = function() {
	frappe.model.get_value("Employee", {"user_id": cur_frm.doc.clerk_name}, "signature", function(r) {
		if(r && !r.exception) {
			...
			cur_frm.set_value("chief_signature_value", r.signature);
			cur_frm.refresh_field("chief_signature_value");
			// } else {
...
cur_frm.cscript.custom_refresh = function() {
	...
	// 	cur_frm.set_value("delivery_date", frappe.datetime.add_days(cur_frm.doc.new_transaction_date, 2));
...
cur_frm.cscript.delivery_period = function() {
	...
	cur_frm.set_value("delivery_date", frappe.datetime.add_days(cur_frm.doc.new_transaction_date, delivery_days));
	cur_frm.refresh_field("delivery_date");
}
