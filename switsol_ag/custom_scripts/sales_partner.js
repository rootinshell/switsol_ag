frappe.ui.form.on("Sales Partner", "refresh", function(frm) {
	if(!cur_frm.doc.__islocal && (cur_frm.doc.call_comming_from && cur_frm.doc.call_comming_from != "" || frappe.route_options)) {
		...
		cur_frm.add_custom_button(__("Anrufe eintragen"), function() {
			cur_frm.cscript.make_log_sales_partner();
		});
	}
	...
	// show log button
		cur_frm.add_custom_button(__("Anrufe anzeigen"), function() {
			cur_frm.cscript.show_logs_sales_partner();
		});
	}
})


cur_frm.cscript.show_logs_sales_partner = function() {
...
cur_frm.cscript.make_log_sales_partner = function() {
	if(frappe.route_options) {
		var args_dict = {
		...
		frappe.call({
			method: "switsol_ag.custom_scripts.sales_partner.make_log",
			args: {
				"args_dict": args_dict
			},
			async: false,
